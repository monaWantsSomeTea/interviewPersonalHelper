...
struct UserResponse: View {
    ...
    @Binding var question: PromptItem
...
//        .onChange(of: self.question.response) { newResponse in
...
//        }
    ...
    init(question: Binding<PromptItem>) {
        ...
        if let promptItem = question.wrappedValue as? PromptItem,
           let response = promptItem.response {
            self._response.wrappedValue = response
        }
    }
}
...
struct UserResponse_Previews: PreviewProvider {
    static var previews: some View {
        ResponseSection(question:
            Binding(
                get: {
                    TopInterviewQuestions().asPromptItems()[0]
                },
                set: { newValue in
